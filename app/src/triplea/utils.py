...
def user_has_role_for_domain(user_id, role_code, domain_id):
    if models.UserDomainRole.objects.filter(user__id=user_id, role__code=role_code, domain__id=domain_id).exists():
        return True

    domain = models.Domain.objects.get(id=domain_id)
    if domain.parent_id:
        return user_has_role_for_domain(user_id, role_code, domain.parent_id)

    return False


def domain_has_permission_for_role(domain_id, permission_code, role_code):

    if models.DomainRolePermission.objects.filter(domain__id=domain_id, role__code=role_code, permission__code=permission_code).exists():
        return True

    # Check acquisition. Only if an item exists and specifically opts out of inherit
    # do we return false.
    cannot_inherit = models.DomainPermission.objects.filter(
    ...
    if domain.parent is not None:
        return domain_has_permission_for_role(domain.parent_id, permission_code, role_code)


def _user_has_permission_for_domain(user_id, permission_code, domain_id, domain_ids):

    # Check for user roles declared on the domain
    user_domain_roles = [
        o.role for o in models.UserDomainRole.objects.filter(user__id=user_id, domain__id__in=domain_ids)
    ]
    ...
    domain = models.Domain.objects.get(id=domain_id)
    if domain.parent is not None:
        return _user_has_permission_for_domain(user_id, permission_code, domain.parent_id, domain_ids+[domain.parent_id])

    return False


def user_has_permission_for_domain(user_id, permission_code, domain_id):
    domain_ids = [domain_id]
    return _user_has_permission_for_domain(user_id, permission_code, domain_id, domain_ids)


def _user_has_permission_for_resource(user_id, permission_code, resource_id, resource_ids):

    # Check for user roles declared on the resource
    user_resource_roles = [
        o.role
        for o in models.UserResourceRole.objects.filter(
    ...
    if user_resource_roles:
        ...
        if for_resource.exists():
            return True

    # Check acquisition. Only if an item exists and specifically opts out of inherit
    # do we return false.
    cannot_inherit = models.ResourcePermission.objects.filter(
    ...
    if resource.parent is not None:
        return _user_has_permission_for_resource(user_id, permission_code, resource.parent_id, resource_ids+[resource.parent_id])
    else:
        ...
        for role in user_resource_roles:
            if domain_has_permission_for_role(resource.domain_id, permission_code, role.code):
                return True
        ...
        for obj in models.ResourceRolePermission.objects.filter(
            resource__id__in=resource_ids, permission__code=permission_code
        ):
            if user_has_role_for_domain(user_id, obj.role.code, resource.domain_id):
                return True

        # Simple recursion
        return user_has_permission_for_domain(user_id, permission_code, resource.domain_id)

    return False


def user_has_permission_for_resource(user_id, permission_code, resource_id):
    resource_ids = [resource_id]
    return _user_has_permission_for_resource(user_id, permission_code, resource_id, resource_ids)


def get_user_domains(user):
    """Return all domains on which the user has any roles, both explicit and inherited.
    """
    # TODO: sqlite can't do distinct. Write two code paths.
    domain_ids = list(set([o.hex for o in models.UserDomainRole.objects.filter(user=user).values_list("domain_id", flat=True)]))
    if domain_ids:
        ...
        return models.Domain.objects.filter(
            id__in=domain_ids + [o.id for o in models.Domain.objects.raw(query)]
        ).order_by("title")
    else:
        return models.Domain.objects.none()


def get_domain_roles(domain):
    found = []
    parent = domain
    while parent is not None:
        for obj in models.DomainRolePermission.objects.filter(domain=parent):
...
def get_domain_permissions(domain):
    found = []
    parent = domain
    while parent is not None:
        for obj in models.DomainPermission.objects.filter(domain=parent):
...
def domain_roles_permissions_mapping(domain):
    """Mapping structure used by views.
    """
    rows = []
    roles = get_domain_roles(domain)
    for permission in get_domain_permissions(domain):
        row = [permission]
        for role in roles:
            active = models.DomainRolePermission.objects.filter(domain=domain, role=role, permission=permission).exists()
...
def get_resource_roles(resource):
    ...
    for role in get_domain_roles(resource.domain):
...
def get_resource_permissions(resource):
    ...
    parent = resource
    while parent is not None:
        for obj in models.ResourcePermission.objects.filter(resource=resource):
            if obj.permission not in found:
                # We cheat a bit by gluing inherit on the permission
                obj.permission.inherit = obj.inherit
                found.append(obj.permission)
    ...
    for permission in get_domain_permissions(resource.domain):
...
def resource_roles_permissions_mapping(resource):
    """Mapping structure used by views.
    """
    rows = []
    roles = get_resource_roles(resource)
    for permission in get_resource_permissions(resource):
        row = [permission]
        for role in roles:
            active = models.ResourceRolePermission.objects.filter(resource=resource, role=role, permission=permission).exists()
            row.append({"permission": permission, "active": active, "role": role})
        rows.append(row)
    return rows
