...
from triplea_api.constants import DEFAULT_ROLES_Q
from triplea_api.utils import paginate_result
from triplea.models import Role
from triplea.serializers import RoleSerializer
...
def post(body, user, token_info, **kwargs):
    ...
    try:
        obj = Role.objects.create(**body)
    except ValidationError as e:
        return e.message_dict, 422

    return RoleSerializer(instance=obj).data, 201
...
def get(code, user, token_info, **kwargs):
    ...
    try:
        obj = Role.objects.get(code=code)
    ...
    return RoleSerializer(instance=obj).data, 200
