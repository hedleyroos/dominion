from uuid import UUID

from django.core.exceptions import ValidationError
from django.db.models import Q
from django.db.models.deletion import ProtectedError
from flask import request

from triplea_api.constants import DEFAULT_PERMISSIONS_Q
from triplea_api.utils import paginate_result
from triplea.models import Permission
from triplea.serializers import PermissionSerializer
from triplea.utils import user_has_permission_for_domain, get_user_domains


ITEM_NOT_FOUND = "Item not found for id: {}."


def post(body, user, token_info, **kwargs):
    user = token_info["user"]

    domain_id = body.pop("domain")
    # Connexion doesn't validate UUID's yet
    try:
        UUID(domain_id)
    except ValueError:
        return {"message": "%s is not a valid UUID." % domain_id}, 400

    if not user_has_permission_for_domain(user.pk, "create", domain_id):
        return {
            "message": "You do not have permission to create this item for this domain."
        }, 403

    body["domain_id"] = domain_id

    # Create an in-memory object so we can run checks without attempting to save to the database.
    # This provides us with clean error messages.
    try:
        obj = Permission(**body)
        obj.full_clean()
    except ValidationError as e:
        if hasattr(e, "error_dict"):
            return e.message_dict, 422
        else:
            return {"message": e.messages[0]}, 422

    # Actually create and persist an object
    try:
        obj = Permission.objects.create(**body)
    except ValidationError as e:
        if hasattr(e, "error_dict"):
            return e.message_dict, 422
        else:
            return {"message": e.messages[0]}, 422

    return PermissionSerializer(instance=obj).data, 201


def get(code, user, token_info, **kwargs):
    user = token_info["user"]

    try:
        obj = Permission.objects.get(code=code)
    except Permission.DoesNotExist:
        return {"message": ITEM_NOT_FOUND.format(code)}, 404

    if not obj.domain_id or not user_has_permission_for_domain(user.pk, "read", obj.domain_id):
        return {"message": "You do not have permission to read this item."}, 403

    return PermissionSerializer(instance=obj).data, 200


def put(code, body, user, token_info, **kwargs):
    user = token_info["user"]

    try:
        obj = Permission.objects.get(code=code)
    except Permission.DoesNotExist:
        return {"message": ITEM_NOT_FOUND.format(code)}, 404

    if not obj.domain_id or not user_has_permission_for_domain(user.pk, "update", obj.domain_id):
        return {"message": "You do not have permission to update this item."}, 403

    for k, v in body.items():
        setattr(obj, k, v)
    try:
        obj.full_clean()
        obj.save()
    except ValidationError as e:
        if hasattr(e, "error_dict"):
            return e.message_dict, 422
        else:
            return {"message": e.messages[0]}, 422

    return PermissionSerializer(instance=obj).data, 200


def delete(code, user, token_info, **kwargs):
    user = token_info["user"]

    try:
        obj = Permission.objects.get(code=code)
    except Permission.DoesNotExist:
        return {"message": ITEM_NOT_FOUND.format(code)}, 404

    if not obj.domain_id or not user_has_permission_for_domain(user.pk, "delete", obj.domain_id):
        return {"message": "You do not have permission to delete this item."}, 403

    try:
        obj.delete()
    except ProtectedError:
        return {
            "message": "Cannot delete item because other items are dependent on it. You must delete those items first."
        }, 422

    return {"message": "Item deleted successfully"}, 204


def search(user, token_info, **kwargs):
    user = token_info["user"]
    # TODO: get_user_domains does not do permission checks. The user needs the read permission 
    # for the domain. That is a separate check that is expensive.
    root_domains = get_user_domains(user).filter(parent__isnull=True)
    return paginate_result(
        Permission.objects.filter(Q(domain__in=root_domains)|DEFAULT_PERMISSIONS_Q).order_by("code"),
        PermissionSerializer,
        request
    )
